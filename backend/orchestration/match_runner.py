...
def run_match(
    ...
    for step in range(max_steps):
        ...
        try:
            decision_result = agent.decide(private_view, data_generation=data_generation)
        ...
        observation = {
            ...
            "agent": {
                ...
                "private_thoughts": decision_result.get("private_thoughts"),
                "public_thoughts": decision_result.get("public_thoughts"),
...
def format_observation_lines(observation: dict) -> list[str]:
    ...
    agent_result = observation.get("agent", {})
    thoughts = agent_result.get("thoughts")
    private_thoughts = agent_result.get("private_thoughts")
    public_thoughts = agent_result.get("public_thoughts")
    ...
    lines = []
    if private_thoughts:
        lines.append(f"> {agent} *private* thoughts: {private_thoughts}")
    elif thoughts:
        lines.append(f"> {agent} thoughts: {thoughts}")

    def finish() -> list[str]:
        if public_thoughts:
            lines.append(f"> {agent} *public* thoughts: {public_thoughts}")
        return lines
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="false",
        type=parse_bool,
        help="Use data-generation prompts and legacy single thoughts output. Defaults to false.",
    )
