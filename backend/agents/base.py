...
When the prompt says data_generation is true, you may include a "thoughts" field
with one concise sentence of strategic rationale.

When the prompt says data_generation is false, include both:
- "private_thoughts": a concise private rationale that only you can see.
- "public_thoughts": a concise message visible to every other player.

Public thoughts must not reveal your private cards, hidden hand information, or
any other information only your player can see. Private thoughts may use your
private view.
...
{"command":"declare","action":"Tax","private_thoughts":"My Duke makes Tax safe and gets me closer to Coup.","public_thoughts":"I am building pressure with coins."}
{"command":"respond","response":"pass","private_thoughts":"I cannot block this claim with my cards.","public_thoughts":"I will let this resolve."}
...
class CoupLLMAgent:
    ...
    def build_user_prompt(
        ...
        thought_instruction = (
            "Because data_generation is true, use the optional thoughts field if you include rationale."
            if data_generation
            else (
                "Because data_generation is false, include private_thoughts and public_thoughts. "
                "public_thoughts will be shown to all other players, so do not reveal your private cards "
                "or hidden information there. private_thoughts will stay private."
            )
        )
        return json.dumps(
            {
    ...
    def _string_field(self, raw_decision: dict, key: str) -> str | None:
    ...
    def _parse_decision_result(self, raw_output: str, private_view: dict) -> dict:
        ...
        thoughts = self._string_field(raw_decision, "thoughts")
        private_thoughts = self._string_field(raw_decision, "private_thoughts")
        public_thoughts = self._string_field(raw_decision, "public_thoughts")
        return {
            "decision": normalize_decision(raw_decision, private_view),
            "thoughts": thoughts,
            "private_thoughts": private_thoughts,
            "public_thoughts": public_thoughts,
    ...
    def decide(self, private_view: dict, data_generation: bool = True) -> dict:
        ...
        return {
            ...
            "private_thoughts": (
                f"I returned an illegal move twice, so the runner used a safe legal fallback: {retry_error}"
            ),
            "public_thoughts": "I need to make a legal fallback move." if not data_generation else None,
            "raw_output": raw_output,
            "fallback": True,
        }
